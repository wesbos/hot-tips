...
async function collect2<DataType>(promise: Promise<DataType>): Promise<[DataType, undefined] | [undefined, Error]> {
  try {
    ...
    if (typeof err === 'string' || typeof err === 'number' || typeof err === 'boolean') {
      return [undefined, new Error(err.toString())];
    }
    // Otherwise we have an object or an array
    const myError = new Error('Promise Rejected', { cause: err });
    return [undefined, myError];
  }
}

const [user2, error2] = await collect2(getUser());
if(error2) { error2; }
else { user2; }

const [sammy2, sammyError2] = await collect2(getSandwich());
if (sammyError2) { sammyError2; }
else { sammy2; }

const [toErr, toUser] = await to(getUser());
if (toErr) {
  toErr;
} else {
  toUser;
...
const [r1,e1] = await collect2(getSandwich());
...
const [user, error] = await collect(getUser());
const [sammy, sammyError] = await collect(getSandwich());
